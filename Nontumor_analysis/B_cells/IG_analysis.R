...
res_con <- as.data.frame(matrix(data = NA, nrow = 10000, ncol = 7))
colnames(res_con) <- c("barcode", "orig.ident",'STK11mut_vs_NonSTK11mut',
                       'PRIMARY_vs_BRAIN_METS', "heavy", "light", "constant")
...
for (c in 1:ncol(tab)) {
  ...
  if (cell[igh] > 0 & cell[igl] > 0 & cell[con] > 0) {
    ...
    res_con$STK11mut_vs_NonSTK11mut[i] <- seu$STK11mut_vs_NonSTK11mut[colnames(seu) == colnames(tab)[c]]
    res_con$PRIMARY_vs_BRAIN_METS[i] <- seu$PRIMARY_vs_BRAIN_METS[colnames(seu) == colnames(tab)[c]]
    i <- i + 1
  }
}
...
res_con<-na.omit(res_con)

# Add IG subtype info
res_con$subtype <- substr(res_con$constant, 1, 4)
res_con <- res_con[!is.na(res_con$light) & !is.na(res_con$heavy), ]
ifelse(!dir.exists(file.path(paste0(path.ct,'IG_analysis'))), 
       dir.create(file.path(paste0(path.ct,'IG_analysis')),recursive = T), FALSE)
write.csv(res_con, paste0(path.ct,'IG_analysis/table_IG_',filename,'_chains.csv'), 
          row.names = F, quote = F)
...
pdf(paste0(path.ct,"IG_analysis/plots_IG_combination_frequency.pdf"), height = 12, width = 12)
...
write.table(lightorder, paste0(path.ct,"IG_analysis/lightorder.txt"), col.names = F, 
            row.names = F, quote = F, sep = "\t")
heavyorder = colnames(res)[out$colInd]
write.table(heavyorder, paste0(path.ct,"IG_analysis/heavyorder.txt"), col.names = F, 
            row.names = F, quote = F, sep = "\t")
...
write.table(lightoveralldf, paste0(path.ct,"IG_analysis/lightoverallfreq.txt"), 
            row.names = F, col.names = T, quote = F, sep = "\t")
write.table(heavyoveralldf, paste0(path.ct,"IG_analysis/heavyoverallfreq.txt"), 
            row.names = F, col.names = T, quote = F, sep = "\t")


# count number of light chain occurrences in each patient
theme_set(theme_bw())
res_patient_tally_light = res_con %>% group_by(light, orig.ident) %>% tally()
res_stk_tally_light = res_con %>% group_by(light, STK11mut_vs_NonSTK11mut) %>% tally()
res_organ_tally_light = res_con %>% group_by(light, PRIMARY_vs_BRAIN_METS) %>% tally()

pdf(paste0(path.ct,"IG_analysis/plots_usage_light_per_patient.pdf"),width = 12,height = 10)
...
ggplot(res_patient_tally_light, 
       aes(fill = orig.ident, x = n, y = factor(light, level = lightoveralldf$lightchain[order(lightoveralldf$frequency)]))) + 
  geom_bar(position = "stack", stat = "identity",colour="black",size=0.25) + 
  ylab("Light Chain") + xlab("Usage")+theme_bw()

ggplot(res_stk_tally_light, 
       aes(fill = STK11mut_vs_NonSTK11mut, x = n, y = factor(light, level = lightoveralldf$lightchain[order(lightoveralldf$frequency)]))) + 
  geom_bar(position = "stack", stat = "identity",colour="black",size=0.25) + 
  ylab("Light Chain") + xlab("Usage")+theme_bw()

ggplot(res_organ_tally_light, aes(fill = PRIMARY_vs_BRAIN_METS, x = n, y = factor(light, level = lightoveralldf$lightchain[order(lightoveralldf$frequency)]))) + 
  geom_bar(position = "stack", stat = "identity",colour="black",size=0.25) + 
  ylab("Light Chain") + xlab("Usage")+theme_bw()
dev.off()
...
res_patient_tally_heavy = res_con %>% group_by(heavy, orig.ident) %>% tally()
res_stk_tally_heavy = res_con %>% group_by(heavy, STK11mut_vs_NonSTK11mut) %>% tally()
res_organ_tally_heavy = res_con %>% group_by(heavy, PRIMARY_vs_BRAIN_METS) %>% tally()

pdf(paste0(path.ct,"IG_analysis/plots_usage_heavy_per_patient.pdf"),width = 12,height = 10)
ggplot(res_patient_tally_heavy, aes(fill = orig.ident, x = n, y = factor(heavy, level = heavyoveralldf$heavychain[order(heavyoveralldf$frequency)]))) + 
  geom_bar(position = "stack", stat = "identity",colour="black",size=0.25) + 
  ylab("Light Chain") + xlab("Usage")+theme_bw()

ggplot(res_stk_tally_heavy, aes(fill = STK11mut_vs_NonSTK11mut, x = n, y = factor(heavy, level = heavyoveralldf$heavychain[order(heavyoveralldf$frequency)]))) + 
  geom_bar(position = "stack", stat = "identity",colour="black",size=0.25) + 
  ylab("Light Chain") + xlab("Usage")+theme_bw()

ggplot(res_organ_tally_heavy, aes(fill = PRIMARY_vs_BRAIN_METS, x = n, y = factor(heavy, level = heavyoveralldf$heavychain[order(heavyoveralldf$frequency)]))) + 
  geom_bar(position = "stack", stat = "identity",colour="black",size=0.25) + 
  ylab("Light Chain") + xlab("Usage")+theme_bw()
dev.off()
...
pdf(paste0(path.ct,'IG_analysis/plots_HL_chains_all_samples.pdf'))
for(pat in unique(res_con2$orig.ident)){
  tab <- res_con2 %>% 
    filter(orig.ident == pat) %>% 
    select(cell_type_fine,heavy_light)
  tab$cell_type_fine <- factor(tab$cell_type_fine,
                               levels = c('Activated B cells',  'Plasma cells','Cycling cells'))
  
  lis_all <- list(
    activated = tab[tab$cell_type_fine=='Activated B cells',2],
    plasma = tab[tab$cell_type_fine=='Plasma cells',2],
    cycling = tab[tab$cell_type_fine=='Cycling cells',2]
    )
  
  ## Plots per patient
  #pdf(paste0(path.ct,'IG_analysis/plots_HL_chains_',pat,'.pdf'))
  # Venn diagram
