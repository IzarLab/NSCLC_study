# Author: Somnath Tagore, Ph.D. Title: inferCNV analysis
# Script Name: infercnv_run.R
...
seu <- readRDS(file = paste0(pat, '.rds'))
...
gene_order <- read.table('refdata-gex-GRCh38-2020-A_gen_pos.txt', header = F,row.names = 1)
...
# Save object
saveRDS(seu, paste0('inferCNV_subcluster_', pat, '_cnv.rds'))

# Plots
pdf(paste0('inferCNV_subcluster_', pat, '_cnv.pdf'))
...
FeaturePlot(seu, features = c('EPCAM', 'JUN', 'proportion_scaled_cnv_avg'),
