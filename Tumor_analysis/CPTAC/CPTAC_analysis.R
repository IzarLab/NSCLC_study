...
# Script Name: CPTAC_analysis.R
# Last Updated: 06/24/2022

#Instructions
# The following script performs Fraction of Genome Altered in CPTAC cohort 
#
# -------------------------
#!/usr/bin/env Rscript
...
# 1) CPTAC LUAD All STK_mut vs All other genes
# -----------------------------

ichorcna_data <- read.csv(file="luad_cptac_2020_clinical_data_2.csv")
...
yplot <- ggboxplot(data_ordered, x = "STK11_MUT_vs_Other_genes", y = "Fraction.Genome.Altered", title = "CPTAC (LUAD): STK11-MUT (All samples) vs STK11-WT (Other gene mutations)",
...
ggsave("Fraction_Genome_Altered_STK11_LUAD_CPTAC_other_genes_v5.pdf",height = 10,width = 10)

# 2) CPTAC LUAD All STK11_mut vs STK11_wt
# -----------------------------

ichorcna_data <- read.csv(file="luad_cptac_2020_clinical_data_2.csv")
ichorcna_data <- as.data.frame(ichorcna_data)
colnames(ichorcna_data)
yplot <- ggboxplot(ichorcna_data, x = "STK11_mut_vs_Non_STK11_mut", y = "Fraction.Genome.Altered", title = "CPTAC (LUAD): STK-mut (All samples) vs Non-STK-mut (All samples)",
...
ggsave("Fraction_Genome_Altered_STK11_NonSTK11_LUAD_CPTAC_other_genes_v3.pdf",height = 15,width = 15)
